        ...
        if let Some(os_instances) = parsed_data.as_array() {
            ...
            for instance in os_instances {
                if let Some(obj) = instance.as_object()
                    && let Some(tabs) = obj.get("tabs").and_then(|v| v.as_array())
                {
                    println!("Tab count: {}", tabs.len());

                    for tab in tabs {
                        if let Some(tab_obj) = tab.as_object()
                            && let Some(windows) = tab_obj.get("windows").and_then(|v| v.as_array())
                        {
                            for window in windows {
                                if let Some(win_obj) = window.as_object() {
                                    ...
                                    if let Some(cmdline) =
                                        ...
                                        && let Some(cmd) = cmdline.first().and_then(|v| v.as_str())
                                    {
                                        println!("  Shell: {}", cmd);
                                    }
                                        ...
                                        .and_then(|v| v.as_array())
                                    {
                                        for proc in procs {
                                            if let Some(proc_obj) = proc.as_object() {
                                                ...
                                                if let Some(pid) =
                                                    proc_obj.get("pid").and_then(|v| v.as_u64())
                                                {
                                                    println!("    PID: {}", pid);
                                                }
                                                ...
                                                if let Some(proc_cmdline) = proc_obj
                                                    .get("cmdline")
                                                    .and_then(|v| v.as_array())
                                                    && let Some(first_arg) = proc_cmdline
                                                        .first()
                                                        .and_then(|v| v.as_str())
                                                {
                                                ...
                                                if let Some(proc_cwd) =
                                                    proc_obj.get("cwd").and_then(|v| v.as_str())
                                                {
